 ...
 * returned from an OAuth 2.0 Resource Server's Metadata Endpoint, and contains a set
 * of claims about the Resource Server's configuration. The claims are defined by the
 * OAuth 2.0 Protected Resource Metadata specification (RFC 9728).
 *
 * @author Joe Grandja
 * @see <a target="_blank" href="https://www.rfc-editor.org/rfc/rfc9728.html#section-2">2. Protected Resource Metadata</a>
 */
public final class OAuth2ProtectedResourceMetadata implements OAuth2ProtectedResourceMetadataClaimAccessor, Serializable {
	...
	public static final class Builder {
		...
		private void validate() {
			...
			if (this.claims.get(OAuth2ProtectedResourceMetadataClaimNames.AUTHORIZATION_SERVERS) != null) {
				...
				Assert.notEmpty((List<?>) this.claims.get(OAuth2ProtectedResourceMetadataClaimNames.AUTHORIZATION_SERVERS),
						"authorization_servers cannot be empty");
				List<?> authorizationServers = (List<?>) this.claims.get(OAuth2ProtectedResourceMetadataClaimNames.AUTHORIZATION_SERVERS);
				authorizationServers.forEach(authorizationServer -> validateURL(authorizationServer, "authorization_server must be a valid URL"));
			}
			...
			if (this.claims.get(OAuth2ProtectedResourceMetadataClaimNames.BEARER_METHODS_SUPPORTED) != null) {
				...
				Assert.notEmpty((List<?>) this.claims.get(OAuth2ProtectedResourceMetadataClaimNames.BEARER_METHODS_SUPPORTED),
